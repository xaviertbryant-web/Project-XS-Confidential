...
class AppViewModel: ObservableObject {
    ...
    init() {
        profile        = DataService.shared.loadProfile()
        goals          = DataService.shared.loadGoals()
        paycheckBudget = DataService.shared.loadPaycheckBudget()

        $profile.sink        { DataService.shared.saveProfile($0) }.store(in: &cancellables)
        $goals.sink          { DataService.shared.saveGoals($0) }.store(in: &cancellables)
        $paycheckBudget.sink { DataService.shared.savePaycheckBudget($0) }.store(in: &cancellables)
    }
